...
function ReceivingReceipt(request, response) {
	...
	for(var x = 1; x <= itemcount; x++) {
	   var arr = [];
	   arr.push(x, ir.getLineItemValue('item', 'description', x));
	   new_array.push(arr);
	}

	var this_array = new_array.sort(cmp);

	for(var i in this_array) {
		item = ir.getLineItemText('item', 'item', this_array[i][0]);
		desc = (ir.getLineItemValue('item', 'description', this_array[i][0]) == null) ? '' : ir.getLineItemValue('item', 'description', this_array[i][0]);
		um = (ir.getLineItemValue('item', 'unitsdisplay', this_array[i][0]) == null) ? '' : ir.getLineItemValue('item', 'unitsdisplay', this_array[i][0]);
		rate = ir.getLineItemValue('item', 'rate', this_array[i][0]);
		amount = 0;

		ir.selectLineItem('item', this_array[i][0]);
